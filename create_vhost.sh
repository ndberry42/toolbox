...
echo "Directory for vhost already exists!"
else
	#Make the vHost directory
echo "Making directory structure"
mkdir -p /var/www/vhosts/${DOMAIN}/
echo "Directory /var/www/vhosts/${DOMAIN}/ Created!"
fi
####
#Make the config
####
...
##Symlink it over from avail to enabled
		...
		if [ -s /etc/apache2/sites-enabled/${DOMAIN}.conf ]; then
			echo "Symlink looks good!"
		else
			echo "There seems to be an issue with the symlink :("
		fi
###Reload Section
